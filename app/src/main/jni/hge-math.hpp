...
//				r.mat[15] = mat[] * m.mat[] + mat[] * m.mat[] + mat[] * m.mat[] + mat[] * m.mat[];
					...
					const element_type &far)
			{
				...
				r.mat[11] = element_type(-1.0);
				r.mat[12] = element_type(0.0);
				r.mat[13] = element_type(0.0);
				r.mat[14] = element_type((2.0 * far * near) / (near - far));
			...
			void setData(std::shared_ptr<utility::Stream> &stream, const bool &endianCompatible = true)
			{
				...
				for (int i = 0; i < 16; i++)
				{
					...
					if (!endianCompatible)
					{
						swapObject((char *)(&v), sizeof(v));
					}
					mat[i] = element_type(v);
#ifdef HGE_VERBOSE_TEST_MODE
					info += "m[" + std::to_string(i) + "]: " + std::to_string(v) + " ";
#endif
				}
#ifdef HGE_VERBOSE_TEST_MODE
				HGE_LOG_PRINT("%s", info.c_str());
#endif
			}
		};
	}
